...
export default function Transcript(props) {
  try {
    ...
    const results = props.messages.map(msg => msg.results.map((result, i) => (
      <>
      <span key={`result-${msg.result_index + i}`}>{result.alternatives[0].timestamps[0][1]}-{result.alternatives[0].timestamps.lastItem[2]}{result.alternatives[0].transcript}</span><br></br>
      </>
